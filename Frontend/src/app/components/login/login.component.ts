...
@Component({
  ...
  template: `
    <div class="auth-wrapper d-flex align-items-center justify-content-center" style="min-height: calc(100vh - 76px);">
      <div class="auth-card glass p-4 shadow-lg animate-fade-in" style="width: 100%; max-width: 400px;">
        <div class="text-center mb-3">
          <div class="auth-icon bg-primary text-white rounded-4 shadow-sm mb-2" style="width: 60px; height: 60px;">
             <i class="bi bi-person-badge-fill fs-2"></i>
          </div>
          <h2 class="fw-bold text-main mb-0">Staff<span class="text-primary">Sphere</span></h2>
          <p class="text-muted small mb-0">Access your workspace</p>
        </div>
        ...
        <form [formGroup]="loginForm" (ngSubmit)="onSubmit()">
          <div class="mb-3">
            ...
            <div class="input-group-custom">
              ...
              <input type="email" formControlName="email" class="form-control-custom" placeholder="admin@emp.com">
            </div>
          </div>
          ...
          <button type="submit" class="btn btn-primary w-100 py-2 rounded-3 shadow-sm d-flex align-items-center justify-content-center gap-2 mb-3" [disabled]="loading">
             <span *ngIf="loading" class="spinner-border spinner-border-sm"></span>
             <span *ngIf="!loading">Continue to Dashboard</span>
          </button>
          
          <div class="text-center mb-3">
            <span class="text-muted small">New to StaffSphere? </span>
            <a routerLink="/register" class="text-primary fw-bold text-decoration-none small ms-1">Create Account</a>
          </div>
  ...
  styles: [`
    ...
    .admin-preview { background: var(--bg-main); border: 1px dashed var(--border-light); }
  `]
})
export class LoginComponent {
  ...
  constructor(
    ...
    this.loginForm = this.fb.group({
      email: ['admin@emp.com', [Validators.required, Validators.email]],
  ...
  onSubmit() {
    ...
    this.auth.login(this.loginForm.value).subscribe({
      ...
      error: () => {
        this.toastr.error('Invalid credentials, please try again', 'Auth Error');
        this.loading = false;
      }
    });
  }
